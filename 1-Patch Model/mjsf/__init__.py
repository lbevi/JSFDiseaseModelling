...
from mjsf.types import Time, SystemState, CompartmentValue, Trajectory
from mjsf import exact
from mjsf import sbml
...
def jsf(x0: SystemState, rates, stoich, t_max, timeShow, **kwargs) -> Trajectory:
    ...
    Args:
        ...
        timeShow: Displays current simulation time every timeShow
    ...
    if kwargs['method'] is None or kwargs['method'] == 'exact':
        result = exact.JumpSwitchFlowExact(x0, rates, stoich, t_max, timeShow, kwargs['config'])
    elif kwargs['method'] == 'operator-splitting':
        result = JumpSwitchFlowSimulator(x0, rates, stoich, t_max, timeShow, kwargs['config'])
    else:
...
def JumpSwitchFlowSimulator(
    ...
    while ContT < t_max:
        ...
        exact.printTime(ContT, timeShow, nextPrint)
