...
def JumpSwitchFlowExact(
    ...
    Args:
        ...
        timeShow: Shows simulation time every timeShow
    ...
    while time < t_max:
        ...
        # Print Every timeShow
        printTime(time, timeShow, nextPrint)
...
def printTime(time: Time, 
              timeShow: Time,
              nextPrint):
    """
    Current simulation time is printed every timeShow
    """

    if timeShow == 0 or timeShow == None:
        return

    if time >= nextPrint[0]:
        print(f"Current Simulation Time, t = {time}")
        nextPrint[0] += timeShow
